...
function(t) {
    ...
    l = ".tplay-tabs",
    s = ".tplay-tab-title",
    c = ".tplay-tabs-toolsbox",
    h = void 0,
    f = void 0,
    u = void 0,
    p = {
        TITLE: ['<li lay-id="{{d.id}}" data-path={{d.path}}>', '<span title="{{d.title}}">', "{{#if(d.icon){}}", '<i class="layui-icon">{{d.icon}}</i> ', "{{#}}}", "{{d.title}}", "</span>", '<i class="layui-icon tplay-tab-close">&#x1006;</i>', "</li>"].join(""),
        CONTENT: ['<div class="tplay-tabs-item" data-path={{d.path}} lay-tab-id="{{d.id}}" data-component="{{d.component}}" data-rendered="{{d.rendered}}">', "<router-view></router-view>", "</div>"].join(""),
        TOOLSITEM: ['<li class="tplay-item" lay-id="{{d.id}}">', '<a href="javascript:;">', '<span title="{{d.title}}">{{d.title}}</span>&nbsp;', '<i class="layui-icon layui-close">&#x1006;</i>', "</a>", "</li>"].join("")
    },
    v = function() {
        this.config = {
    ...
    v.prototype.render = function(t) {
        var e = this,
        a = i(l).attr("tplay-target");
        return f = void 0 === f ? i(s) : f,
        h = void 0 === h ? i('div.tplay-tabs-content[tplay-tabs="' + a + '"]') : h,
        ...
        f.children("li").each(function(t, n) {
            ...
            function(t) {
                layui.stope(t);
                var n = i(this).parents(".tplay-item").attr("lay-id");
                e.remove(n)
            })
        }),
        C.prevHandler(),
    ...
    switch = function(t) {
        ...
        d = f.parent("div.tplay-tab").width();
        if (o > d);
    ...
    var C = {
        ...
        prevHandler: function() {
            i(".tplay-tabs-prev").off("click").on("click",
            function() {
        ...
        nextHandler: function() {
            i(".tplay-tabs-next").off("click").on("click",
            function() {
                var t = f[0].style.left,
                e = parseInt(t.substr(0, t.indexOf("px"))),
                n = i(l).width(),
        ...
        toolHandler: function() {
            i(".tplay-tabs-tools").off("click").on("click",
            function(t) {
                layui.stope(t),
                i(c).show(),
                i(document).on("click",
        ...
        toolBoxHandler: function(t) {
            var e = this;
            i(c).find("li.tplay-item").each(function() {
                var n = i(this),
                a = n.data("action"),
                r = n.attr("lay-id");
        ...
        findContent: function(t) {
            return h.children('div.tplay-tabs-item[lay-tab-id="' + t + '"]')
        },
        ...
        findTools: function(t) {
            return u.find('li.tplay-item[lay-id="' + t + '"]')
        },
        getCurrId: function() {
